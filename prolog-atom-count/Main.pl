empty([]).
removeSubList([],[]):- !.
removeSubList([X|Y],R):- is_list(X), not(empty(X)), removeSubList(X,Z),
                         removeSubList(Y,W), append(Z,W,R), !.
...
conta_atomos(L1, L2, Lout):- removeSubList(L1,L1rem), removeSubList(L2,L2rem),
                             removeVars(L1rem,L1mod), removeVars(L2rem,L2mod), concatSharedElements(L1mod, L2mod, L), countElem(L,Lout).
